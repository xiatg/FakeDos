               ...
               vector<PCB_type> &readyQueue)
{
   ...
   if (!first_exe)
   {
       ...
       int num;
       num = stoi(task_data_read(taskid, "number", jsonmem));

      // retrieve the data with the given taskid
      for (int i = 0; i < num; i++)
      {
         string name = task_data_read(taskid, "name" + to_string(i), jsonmem);
         string remark = task_data_read(taskid, "remark" + to_string(i), jsonmem);
         int state = stoi(task_data_read(taskid, "state" + to_string(i), jsonmem));
         thing.name =name;
         thing.remark = remark;
         thing.state = state;
         list_for_all.push_back(thing);
      }
      cout << "Welcome BACK to your todo list!" << endl;
   ...
   else
   {
      ...
      for (int i = 0; i < list_for_all.size(); i++)
      {
         ...
         bool f1 = task_data_write(taskid, "remark" + to_string(i), list_for_all[i].remark, user_name, username, jsonmem);
         bool f2 = task_data_write(taskid, "state" + to_string(i), to_string(list_for_all[i].state), user_name, username, jsonmem);
         bool f3 = task_data_write(taskid, "number",to_string(list_for_all.size()),user_name, username, jsonmem);

         if ((f0 == false) || (f1 == false) || (f2 == false)||(f3 == false))
         {
            flag = false;
            break;
         }
